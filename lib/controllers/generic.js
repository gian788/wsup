...
var controllerAction = require('./controllerActions');
...
add = function(type, data, sqlCallback){
	...
	if(data.p && data.p.rel)
		_parent['rel'] =data.p.rel;
	mod.create(data.o, function(err, user){
		if(err) return sqlCallback(err, null);
		controllerAction.executeActions(type, user, function(e, r){
			if(e) {
				return sqlCallback(e, null);
			}
			//console.log(user);
			sqlCallback(null, user);
		})
